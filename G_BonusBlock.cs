...
namespace MoreMountains.CorgiEngine
{
	...
	public class G_BonusBlock : CorgiMonoBehaviour, Respawnable
	{
		...
		public float BlockMoveUpDistance = 3f;  // How far the block moves up
		public float BlockMoveSpeed = 0.05f;      // How fast the block moves up and down
		...
		public virtual void OnTriggerEnter2D(Collider2D collider)
		{
			...
			{
				// if the collider's y position is less than the block's y position, we're hitting it from below
				_hit = true;
				_numberOfHitsLeft--;

				// Move block upwards and back down
				...
				/* ***************************CHECK FOR COIN TEST - DELETE LATER - this triggers the coin!!! *********************** */
				// Check if the spawned object has the CoinAnimator script
				CoinAnimator coinAnimator = spawned.GetComponent<CoinAnimator>();
				 /*if (coinAnimator != null)
				{
					Debug.Log("G_BonusBlock: CoinAnimator found, calling AnimateCoin()");
					coinAnimator.AnimateCoin();
				}
				else
				{
					Debug.LogError("G_BonusBlock: No CoinAnimator found on the spawned object");
				} 

				/* ***************************CHECK OVER DELETE LATER *********************** */

				if (AnimateSpawn)
				{
					StartCoroutine(MMMovement.MoveFromTo(spawned, transform.position,
						new Vector2(transform.position.x + SpawnDestination.x,
						transform.position.y + _boxCollider2D.size.y + SpawnDestination.y),
