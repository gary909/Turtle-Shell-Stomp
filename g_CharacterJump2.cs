...
// This modified jump script will include a check that stops the player from hitting two bricks at once.
//  It will work in conjunction with the BrickInteractionHandler.cs code (Also attached to the player).
...
namespace MoreMountains.CorgiEngine
{
	...
	public class CharacterJump : CharacterAbility
	{
		...
		//  ********************************** Check For double brick hits **GW*********************
		private bool hasHitBrick = false; // Flag to track if a brick has been hit //  **************************__TESTING__*****************************


		// Add the OnCollisionEnter2D function (New Function)
		private void OnCollisionEnter2D(Collision2D collision)
		{
			// Check if the collision is with one of the brick objects by name
			if (collision.gameObject.name == "HitCoinFromBelow" ||
				collision.gameObject.name == "G_Block_Energy" ||
				collision.gameObject.name == "G_Block_secretCoin" ||
				collision.gameObject.name == "G_Block" ||
				collision.gameObject.name == "g_Brick")
			{
				if (!hasHitBrick)
				{
					InteractWithBrick(collision.gameObject);
					hasHitBrick = true;
					Debug.Log("you've hit a brick GW");
				}
			}
		}

		// Add the InteractWithBrick function (New Function)
		private void InteractWithBrick(GameObject brick)
		{
			// Your code for interacting with the brick (e.g., spawning a coin, moving the brick)
			Debug.Log("Brick hit: " + brick.name);
			// Example: call a method on the brick to break or spawn a coin
			// brick.GetComponent<Brick>().Break();
		}

		//  ********************************** END OF - Check For double brick hits **GW*********************
		...
		public override void ProcessAbility()
		{
			...
			if (_controller.State.IsGrounded && hasHitBrick)
			{
				hasHitBrick = false;
			}
			// *************************************************___TESTING___***********************************
		...
		public virtual void JumpStart()
		{
			...
			// ******************************__TESTING__****************************************
			// Reset the hasHitBrick flag when the player starts a jump
			hasHitBrick = false;
			// ******************************__TESTING__****************************************
