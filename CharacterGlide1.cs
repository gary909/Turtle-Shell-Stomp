...
// Edited from original MM script...  see updated HandleInput() method
//	V1 Adds glides ability to mobile.
...
namespace MoreMountains.CorgiEngine
{
	...
	public class CharacterGlide : CharacterAbility
	{
		...
		//****************************START OF EDITED HANDLE INPUT METHOD FOR MOBILE CONTROL ***********************************************

		protected override void HandleInput()
		{
			base.HandleInput();

			// Ensure Glide is only available when enabled
			if (!AbilityAuthorized)
			{
				return;
			}

			// Prioritize Jump First - Glide should not interfere
			if (_controller.State.IsGrounded)
			{
				return; // Prevent Glide from interfering with Jump
			}

			// Only allow gliding if the player is FALLING (not going upwards)
			bool isFalling = (_controller.Speed.y <= 0);

			// Check if Jump Button is being HELD to Glide
			if (isFalling && _inputManager.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonPressed)
			{
				GlideStart();
			}

			// Stop Gliding when Jump Button is RELEASED
			if (_inputManager.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonUp && _gliding)
			{
				GlideStop();
			}
		}
		...
		//****************************END OF EDITED HANDLE INPUT METHOD FOR MOBILE CONTROL *************************************************
